...
Steam 桌面配置与 Talaria 同时生效时，可能出现**单击变成双击**、滚动过快或触觉叠加。例如，同一个按键或触摸板按压在两边都绑定鼠标左键，一次操作就可能输出两次点击；请避免这类绑定冲突。**Talaria 的接管不会自动关闭 Steam 桌面配置；白名单也只限制 Talaria，不会阻止 Steam 输出。**

推荐按使用场景选择：

- **非 Steam 游戏**：退出 Steam，启用 Talaria；游戏需要手柄输入时，开启该配置的 Xbox 模拟。
- **Steam 游戏 + 管理员权限修改器**：Steam 保持普通权限，Talaria 安装并启用管理员窗口操作能力（UIAccess），白名单只加入修改器 EXE。游戏交给 Steam Input，修改器交给 Talaria；普通权限 Steam 的键鼠注入受权限边界限制，无法操作该管理员窗口。修改器配置无需开启 Xbox 模拟；触觉仍可能叠加，详见 FAQ。
- **Talaria 负责普通桌面**：先备份 Steam 桌面布局，再使用空布局或移除重叠的按键、触摸板与触觉设置。保留游戏和大屏模式配置，无需全局关闭 Steam Input。清空的桌面布局不会在 Talaria 暂停后自动恢复。

具体设置、白名单的边界以及独占与拔插提示音的关系，见 [FAQ：Steam 桌面配置冲突](docs/faq.md#steam-desktop-conflicts)。
