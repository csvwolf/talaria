...
internal sealed partial class PadHop
{
 ...
 void BuildAbout(bool live){
  ...
  AboutButton(logs,L.T("打开最近日志"),delegate{var files=Directory.GetFiles(AppPaths.Data,"*.txt",SearchOption.TopDirectoryOnly).Concat(Directory.GetFiles(AppPaths.Logs,"*.txt")).Concat(Directory.GetFiles(AppPaths.Logs,"*.log")).Select(f=>new FileInfo(f)).OrderByDescending(f=>f.LastWriteTimeUtc).ToList();if(files.Count==0){Notice(L.T("还没有日志。复现问题后再打开。"));return;}ShowLogViewer(L.T("最近日志"),delegate{return ReadLogText(files[0].FullName);});});
  AboutButton(logs,L.T("打开设备诊断日志"),delegate{string diagnostic=System.IO.Path.Combine(AppPaths.Logs,"devices-diagnostic.log");if(!File.Exists(diagnostic))RefreshDevices();ShowLogViewer(L.T("设备诊断日志"),delegate{return ReadLogText(diagnostic);});});
