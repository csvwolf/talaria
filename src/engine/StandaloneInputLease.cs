...
// Only selected SC2 BLE, USB or active receiver slot, during input takeover with Steam absent.
...
internal sealed class StandaloneInputLease:IDisposable {
 ...
 void Run(){SafeFileHandle file=null;bool changed=false;byte id=0;int length=0;try{
  if(SteamRunning()){State=2;Probe.FirmwareStatus("steam-owned",null);return;}
  ...
  while(!stop.WaitOne(0)){
   if(SteamRunning()){State=2;Probe.FirmwareStatus("steam-owned",null);break;}
   changed=true;Write(file,Command(length,id,0x81));Write(file,Command(length,id,0x87));if(State!=1)Probe.FirmwareStatus("raw-active",null);State=1;
   if(stop.WaitOne(1000))break;
  }
 }catch(Exception e){State=-1;Probe.FirmwareStatus("raw-failed",e);}
 finally{
  // Steam owns its own settings once running; never overwrite them with defaults.
  if(changed && file!=null && !file.IsInvalid && !SteamRunning())try{Write(file,Command(length,id,0x85));Write(file,Command(length,id,0x8e));Probe.FirmwareStatus("default-restored",null);}catch(Exception e){State=-1;Probe.FirmwareStatus("restore-failed",e);}
 ...
 internal static void Test(){foreach(bool active in new[]{false,true})foreach(bool helper in new[]{false,true})foreach(bool preview in new[]{false,true})foreach(bool steam in new[]{false,true})if(BridgePreview.RawModeAllowed(active,helper,preview,steam)!=(active && helper && !preview && !steam))throw new Exception("Raw mode ownership gate");var c=Command(64,1,0x87);if(c[0]!=1||c[1]!=0x87||c[2]!=6||c[3]!=8||c[4]!=0||c[5]!=0||c[6]!=7||c[7]!=0||c[8]!=0)throw new Exception("Raw pad mode payload");if(Command(64,1,0x81)[2]!=0 || Command(64,2,0x85)[1]!=0x85)throw new Exception("Mapping command framing");bool denied=false;try{Command(64,0x82,0x81);}catch(ArgumentException){denied=true;}if(!denied)throw new Exception("Haptics report confused with feature command");}
