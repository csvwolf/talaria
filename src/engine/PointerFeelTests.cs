...
internal static class PointerFeelTests {
 ...
 static void TransportMappingTest(){
  var b=new PadBook{GamepadEnabled=true};byte[] ble=new byte[48];ble[0]=0x45;uint mask=(1u<<6)|(1u<<14)|1u;Array.Copy(BitConverter.GetBytes(mask),0,ble,2,4);Array.Copy(BitConverter.GetBytes((short)12345),0,ble,10,2);Array.Copy(BitConverter.GetBytes((short)18000),0,ble,8,2);var usb=(byte[])ble.Clone();usb[0]=0x42;uint a,c;if(Decoder.Decode(ble,out a)==null || Decoder.Decode(usb,out c)==null || a!=c)throw new Exception("Transport buttons differ");var x=VirtualGamepad.ConvertReport(b,ble,a);var y=VirtualGamepad.ConvertReport(b,usb,c);if(x.Buttons!=y.Buttons || x.LX!=y.LX || x.RT!=y.RT)throw new Exception("Transport Xbox mapping differs");
  using(var l=new DualPads(new PadBook()))using(var q=new DualPads(new PadBook())){for(int i=0;i<4;i++){ble=Pointer(i*800,0,i==2);usb=(byte[])ble.Clone();usb[0]=0x42;if(string.Join(";",l.Step(ble,IntPtr.Zero,IntPtr.Zero,false).ToArray())!=string.Join(";",q.Step(usb,IntPtr.Zero,IntPtr.Zero,false).ToArray()))throw new Exception("Transport pointer mapping differs");}}
 }
 internal static void Run(){TransportMappingTest();ClickToleranceTest();HardwareClickTest();ScrollNoiseTest();var b=new PadBook();b.Left.FineScroll=true;b.Left.ScrollUnits=6000;using(var d=new DualPads(b)){d.Step(Report(0,true),IntPtr.Zero,IntPtr.Zero,false);int sum=0;for(int i=1;i<=100;i++)sum+=Wheel(d.Step(Report(i*10,true),IntPtr.Zero,IntPtr.Zero,false));if(sum<19 || sum>20)throw new Exception("Fine scrolling lost fractional movement");if(Wheel(d.Step(Report(500,true),IntPtr.Zero,IntPtr.Zero,false))>=0)throw new Exception("Scroll reversal failed");d.Step(Report(0,false),IntPtr.Zero,IntPtr.Zero,false);if(Wheel(d.Step(Report(25000,true),IntPtr.Zero,IntPtr.Zero,false))!=0)throw new Exception("Retouch scroll jump");}
